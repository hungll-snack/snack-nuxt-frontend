<template>
      ...
      <v-row>
        ...
        <v-col cols="12" md="4">
          <v-card class="pa-4">
            ...
            <v-img v-if="previewImage" :src="previewImage" class="thumbnail-preview mt-2" />
  ...
  const submitBoard = async () => {
    ...
    try {
      ...
      if (selectedRestaurant.value) formData.append('restaurant', selectedRestaurant.value);
  
      const { djangoAxiosInstance } = axiosUtility.createAxiosInstances();
      await djangoAxiosInstance.put(`/board/update/${boardId}/`, formData, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
