...
export const naverAuthenticationAction = {
    async requestNaverLoginToDjango(): Promise<void> {
        ...
        try {
            return djangoAxiosInstance.get('/naver-oauth/request-login-url').then((res) => {
                console.log(`res: ${res}`)
    ...
    async requestAccessToken(code: string): Promise<string | null> {
        const { djangoAxiosInstance } = axiosUtility.createAxiosInstances();
        try {
            const response = await djangoAxiosInstance.post('/naver-oauth/redirect-access-token', code)
